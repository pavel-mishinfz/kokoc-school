...
if (!$link) {
    ...
    $sql = "SELECT t.name, t.dt_deadline, t.status_ext, t.project_id FROM tasks t JOIN users u ON t.user_id = u.id WHERE u.id = '$user_id'";
    $result = mysqli_query($link, $sql);
    if(!$result) {
        $error = mysqli_error($link);
        print("Ошибка SQL-запроса на чтение: " . $error);
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') { // если для доступа к файлу был произведен метод POST
        ...
        if(count($errors)) {
            ...
            $stmt = db_get_prepare_stmt($link, $sql, $form_project);
